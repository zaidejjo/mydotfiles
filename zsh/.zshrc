...
# --color=marker:#f5e0dc,fg+:#cdd6f4,prompt:#cba6f7,hl+:#f38ba8"
#
smart_preview="if [ -d {} ]; then eza --tree --color=always --icons {} | head -200; else batcat -n --color=always --line-range :500 {}; fi"
show_file_or_dir_preview="if [ -d {} ]; then yazi --chooser-file=/dev/null {}; else batcat -n --color=always --line-range :500 {}; fi"
...
# eval "$(tv init zsh)"
# --- إعدادات fzf الأسطورية ---

# إعداد المعاينة (Preview) للملفات والمجلدات
# يستخدم batcat للملفات و eza للمجلدات
export FZF_CTRL_T_OPTS="
  --preview 'if [ -d {} ]; then eza --tree --color=always {} | head -200; else batcat -n --color=always --line-range :500 {}; fi'
  --bind 'ctrl-/:change-preview-window(down|hidden|)'"

# وظيفة لفتح الملف المختار في nvim فوراً عند الضغط على Ctrl+P
_fzf_open_in_nvim() {
    # هنا تم استبدال batcat بـ smart_preview لحل مشكلة المجلدات
    local file=$(fdfind --hidden --strip-cwd-prefix --exclude .git | fzf --reverse --preview "$smart_preview")
...
# وظيفة البحث الذكي في التاريخ (History) مع معاينة
...
_fzf_grep_nvim() {
    local res=$(rg --column --line-number --no-heading --color=always --smart-case --glob '!.git/' "" | \
                fzf --ansi --delimiter : --preview 'batcat --color=always --highlight-line {2} --style=numbers {1}' --preview-window 'up,60%')
    if [ -n "$res" ]; then
